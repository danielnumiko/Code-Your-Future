...
import { FadeIn, EyebrowMark, GlitchHeading } from './FadeIn'
import { GREEN, PixelStatNumber } from './BrandAssetsV4'
const RED = GREEN
...
export default function PipelineStrategyV1() {
    ...
    <section className="bg-black relative overflow-hidden">
      <div className="max-w-viewport mx-auto px-margins pt-slice pb-slice">
        ...
        <FadeIn className="text-center mb-2xl">
          ...
          <GlitchHeading
            as="h2"
            className="font-raleway font-medium text-h3 tracking-tight leading-tight mb-l max-w-xl mx-auto"
            segments={[
              { text: '[', color: RED },
              { text: 'The tech talent pipeline', color: 'white' },
              { text: ']', color: RED },
              { text: ' ', color: 'white' },
              { text: '<', color: RED },
              { text: "you've", color: 'white' },
              { text: '.', color: RED },
              { text: 'been', color: 'white' },
              { text: '.', color: RED },
              { text: 'missing', color: 'white' },
              { text: '>', color: RED },
            ]}
          />
          <p className="font-raleway text-white/60 text-base leading-relaxed max-w-xl mx-auto">
              ...
              >
                <div className="w-full">
                  {imageLeft ? (
                    ...
                    <div>
                      <div className="mb-s">
                        <PixelStatNumber label={String(i + 1).padStart(2, '0')} />
                      </div>
                      <h3 className="font-raleway font-medium text-white text-h5 leading-snug mb-s">{step.title}</h3>
                      <p className="font-raleway text-white/60 text-base leading-relaxed">{step.body}</p>
                    </div>
                  )}
                </div>
        ...
        <FadeIn className="flex flex-col sm:flex-row items-center justify-center gap-s pt-2xl">
          {ctaLinks.map(link => (
            <a
              ...
              style={
                link.filled
                  ? { background: RED, color: 'black', padding: '14px 24px', borderRadius: 0, display: 'inline-block', fontSize: 18 }
                  : { background: 'transparent', border: '2px solid white', color: 'white', padding: '12px 22px', borderRadius: 0, display: 'inline-block', fontSize: 18 }
              }
            >
              {link.label}
            </a>
