...
export default function BlogPreview() {
  ...
  // Card width = 1/3 of the content area (same grid as the rest of the page).
  // Starting at the left margin means the 4th card bleeds past the right margin.
  const cardW = `calc((100vw - 2 * clamp(20px, calc(8px + 3.75vw), 80px)) / ${visible})`

  return (
    <section className="bg-cyf-offwhite overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-s flex-shrink-0 ml-l">
              <button
                ...
                disabled={offset === 0}
                aria-label="Previous"
                className="w-10 h-10 rounded-full border border-cyf-black/20 flex items-center justify-center hover:border-cyf-black transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
              >
      ...
      {/* Carousel — starts at left margin, bleeds off the right viewport edge */}
      <div
        className="pb-slice"
        ...
        >
          {posts.map(post => (
            ...
            >
              ...
              <p className="font-raleway text-xs mb-s">
                <span className="text-cyf-red font-medium">/ </span>
                <span className="text-cyf-ink">{post.category}</span>
              </p>
              <h3 className="font-raleway font-medium text-cyf-black text-h5 leading-snug group-hover:text-cyf-red transition-colors">
                {post.title}
              </h3>
            </div>
          ))}
